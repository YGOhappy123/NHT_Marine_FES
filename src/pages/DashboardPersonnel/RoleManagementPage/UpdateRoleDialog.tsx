...
const UpdateRoleDialog = ({ role, permissions, updateRoleMutation, open, setOpen }: UpdateRoleDialogProps) => {
    const form = useForm<z.infer<typeof updateRoleFormSchema>>({
    ...
    const onSubmit = async (values: z.infer<typeof updateRoleFormSchema>) => {
        if (!role) return
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="min-w-2xl md:min-w-3xl">
                <DialogHeader>
                    <DialogTitle>Cập nhật vai trò nhân viên</DialogTitle>
                    <DialogDescription>
